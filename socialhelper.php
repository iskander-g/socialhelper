<?php
  class SocialHelper 
  {
     ...
     const VK_SHARE_URL         = 'http://vk.com/share.php?url=%1$s&noparse=%2$s&title=%3$s&description=%4$s&image=%5$s';
     ...
     {
         return sprintf(self::TW_SHARE_URL, $url, $text, $via, $hashtags);
     }
     ...
     {
         return sprintf(self::FB_SHARE_URL, $url);
     }
     ...
     {       
          $data = file_get_contents(sprintf(self::FB_SHARES_COUNT_URL, $url));
          try {
      ...
      * @return string
      */      
     public static function getVkShareUrl($url, $noparse='false', $title, $desc, $image) 
     {
         if($noparse === true || $noparse == 'true') {
            $noparse = 'true';
            return sprintf(self::VK_SHARE_URL, $url, $noparse, $title, $desc, $image);
         } else {
            return sprintf(self::VK_LIGHT_SHARE_URL, $url);
         }
     }      
      ...
      */          
      ...
      {       
          $data = file_get_contents(sprintf(self::VK_SHARES_COUNT_URL, $url));          
     ...
     {
         return sprintf(self::OK_SHARE_URL, $url);
     }
      ...
      */       
      ...
      {       
          $data = file_get_contents(sprintf(self::OK_SHARES_COUNT_URL, $url));
     ...
     {
         return sprintf(self::MR_SHARE_URL, $url, $title, $desc, $image);
     }      
